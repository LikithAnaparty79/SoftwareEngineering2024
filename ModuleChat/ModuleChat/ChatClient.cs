...
namespace ChatApplication
{
    public class ChatClient : INotificationHandler
    {
        ...
        {
            var result = _communicator.Start(serverIP, serverPort);
            if (result == "success")
            {
                //string formattedMessage = $"{message}|{Username}|{recipientId}";
                ...
                //SendMessage($"{Username} has connected to the chat.", "public");
            }
            else
            {
        ...
        public void OnDataReceived(string serializedData)
        {
            ...
            if (dataParts[0] == "clientlist")
            {
                ...
                //_clientList = new List<string>(dataParts[1].Split(','));
                //ClientListUpdated?.Invoke(this, _clientList);
            }
